...
class Actor(object):
    ...
    This algorithm is linear without biases, however it can easily be extended to non-linear case.
    """
    ...
    def __init__(self, n_features):
        ...
        with self._graph.as_default():
            ...
            with tf.name_scope('value_function'):
                _W1 = tf.Variable(tf.truncated_normal((n_features, 1))*0.0001,
                                  dtype=tf.float32, name='weights')
                _l1 = tf.matmul(self.state, _W1, name='layer1')
                self._value_fct = tf.identity(_l1, name='value_function')
